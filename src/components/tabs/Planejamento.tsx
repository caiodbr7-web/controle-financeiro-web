...
import { Kpi, Select, Seg, Toolbar, Panel, Legenda } from "../ui";
...
export function Planejamento({ lancamentos }: { lancamentos: Lancamento[] }) {
  ...
  return (
    <div>
      ...
      {view === "mes" ? (
        <div className="mt-2">
        {!temOrcCartao && <div className="text-amber text-[12px] leading-relaxed mb-1">{MSG_MIGRACAO_CARTAO}</div>}
        {!temOrcConta && <div className="text-amber text-[12px] leading-relaxed mb-1">{MSG_MIGRACAO_CONTA}</div>}
        {!temOrcReceita && <div className="text-amber text-[12px] leading-relaxed mb-1">{MSG_MIGRACAO_RECEITA}</div>}
        <Legenda>
